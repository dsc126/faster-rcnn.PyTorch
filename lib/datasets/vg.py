    ...
    def _do_python_eval(self, output_dir, pickle=True, eval_attributes = False):
        ...
        for i, cls in enumerate(classes):
            if cls == '__background__' or cls == '__no_attribute__':
                continue
            filename = self._get_vg_results_file_template(output_dir).format(cls)
            rec, prec, ap, scores, npos, acc, iou = vg_eval(
            ...
            print('ACC for {} = {:.4f} (npos={:,})'.format(cls, acc, npos))
            print('IOU for {} = {:.4f} (npos={:,})'.format(cls, iou, npos))
            if pickle:
                with open(os.path.join(output_dir, cls + '_pr.pkl'), 'wb') as f:
                    pickle.dump({'rec': rec, 'prec': prec, 'ap': ap,
                        'scores': scores, 'npos':npos, 'acc': acc, 'iou': iou}, f)
        ...
        print('Weighted Mean ACC = {:.4f}'.format(np.average(accs, weights=weights)))
        print('Mean IOU = {:.4f}'.format(np.mean(ious)))
        print('Weighted Mean IOU = {:.4f}'.format(np.average(ious, weights=weights)))
