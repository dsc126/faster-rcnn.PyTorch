  ...
  CUDA_1D_KERNEL_LOOP(index, nthreads) {
    ...
    for (int iy = 0; iy < roi_bin_grid_h; iy ++) // e.g., iy = 0, 1
    {
      ...
      for (int ix = 0; ix < roi_bin_grid_w; ix ++)
      {
        ...
        {
          atomicAdd((int*)(offset_bottom_diff + y_low * width + x_low), (int)static_cast<T>(g1));
          atomicAdd((int*)(offset_bottom_diff + y_low * width + x_high), (int)static_cast<T>(g2));
          atomicAdd((int*)(offset_bottom_diff + y_high * width + x_low), (int)static_cast<T>(g3));
          atomicAdd((int*)(offset_bottom_diff + y_high * width + x_high), (int)static_cast<T>(g4));
        } // if
      } // ix
    } // iy
  } // CUDA_1D_KERNEL_LOOP
